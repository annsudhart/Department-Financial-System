...
/*  VALIDATION & CLEANUP **************************************************************/
IF OBJECT_ID('ql.operating_ledger','V') IS NOT NULL DROP VIEW ql.operating_ledger;
GO

/*  PROCEDURE CREATION ****************************************************************/
PRINT '** CREATE VIEW';
GO
CREATE VIEW    ql.operating_ledger
                AS                            
                SELECT  ifoapal.full_accounting_period,
                        oh3.orghier_level3,
                        ...
                        ifoapal.pa_account,
                        CASE    WHEN SUBSTRING(ifoapal.pa_account,1,1) = '5' THEN '5x'
                        ...
                        (CASE   WHEN cdo.project_type_short IS NULL 
                                THEN 'CDO' 
                        ...
                        tm.team_id,
                        tm.team_name,
                        CASE    WHEN    la.la_amount IS NULL
                                        THEN 0.00
                        ...
                        coat.coa_account,
                        coa.account_label 
                FROM    ga.f_ifoapal AS ifoapal
                        INNER JOIN  ga.f_ledger_activity        AS la   ON  ifoapal.if_id = la.if_id
                                                                        AND ifoapal.full_accounting_period = la.full_accounting_period  
                        INNER JOIN  ga.f_ledger_transaction     AS lt   ON  lt.lt_id = la.lt_id
                        ...
                        LEFT OUTER JOIN ga.f_period_index       AS fp   ON  ifoapal.full_accounting_period = fp.full_accounting_period
                                                                        AND ifoapal.pi_account_index = fp.pi_account_index
                        LEFT OUTER JOIN ga.f_period_fund        AS pf   ON  ifoapal.full_accounting_period = pf.full_accounting_period
                        ...
                        LEFT OUTER JOIN ga.f_period_organization AS po  ON  ifoapal.full_accounting_period = po.full_accounting_period  
                                                                        AND ifoapal.po_organization = po.po_organization
                        LEFT OUTER JOIN ga.f_period_location    AS pl   ON  ifoapal.full_accounting_period = pl.full_accounting_period
                                                                        AND ifoapal.pl_location = pl.pl_location
                        LEFT OUTER JOIN ga.f_period_account     AS pa   ON  ifoapal.full_accounting_period = pa.full_accounting_period
                        ...
                        LEFT OUTER JOIN ga.f_period_program     AS pp   ON  ifoapal.full_accounting_period = pp.full_accounting_period
                                                                        AND ifoapal.pp_program = pp.pp_program
                        ...
                        INNER JOIN coa_db.account               AS ac   ON  ifoapal.pa_account = ac.account
                        ...
                        LEFT OUTER JOIN coa_db.indx             AS i    ON  ifoapal.pi_account_index = i.indx
                                                                        AND i.most_recent_flag = 'Y'
                        LEFT OUTER JOIN fin.xref_mission        AS xref ON  ifoapal.pf_fund = xref.pf_fund
                        ...
                        LEFT OUTER JOIN xref.index_project_type AS xcdo ON  ifoapal.pi_account_index = xcdo.indx
